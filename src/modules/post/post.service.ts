...
export class PostService {
  ...
  async useLike(data: LikeDto) {
    ...
    if (!like) {
      ...
      if (users && queryUser) {
        let newNotification = {
          body: users[0]?.name + ' đã thích bài viết của bạn',
          title: users[0]?.name + ' đã thích bài viết của bạn',
  ...
  async addComment(userId, data: AddCommentDto) {
    ...
    if (user && queryUser) {
      let newNotification = {
        body: user[0]?.name + ' đã bình luận bài viết của bạn',
        title: user[0]?.name + ' đã bình luận bài viết của bạn',
        senderId: userId,
        userId: queryUser[0]?.userId,
        type: NotificationType.LIKE_POST,
        metadata: null,
